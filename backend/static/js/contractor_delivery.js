(function () {
  ...
  function initialiseDelivery(form) {
    ...
    function refreshAddButton() {
      if (!addButton) return;
      const full = rowCount() >= maximum;
      addButton.hidden = maximum <= 0 || full;
    ...
    function bindRow(row) {
      ...
      if (removeButton) {
        removeButton.addEventListener("click", function () {
          row.remove();
          refreshAddButton();
          if (rowCount() === 0 && maximum > 0) addRow();
        });
      }
      ...
      if (input) {
        input.addEventListener("change", function () {
          ...
          if (filename) filename.textContent = file ? file.name : "No photo selected";
          if (!preview) return;
          if (!file) {
    ...
    form.addEventListener("submit", function () {
      ...
      submitButton.innerHTML = '<span class="spinner-border spinner-border-sm" aria-hidden="true"></span> Sending to audit…';
    });
  }
